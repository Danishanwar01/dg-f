...
const BillModal = ({ show, handleClose, sale }) => {
  ...
  doc.text(`Date: ${new Date(sale.created).toLocaleDateString()}`, 150, 40);
  doc.text(`Customer: ${sale.customer?.name || 'N/A'}`, 10, 47);
  doc.text(`Payment Status: ${sale.paymentStatus}`, 10, 54);
  doc.text(`Notes: ${sale.notes || '-'}`, 10, 61);
